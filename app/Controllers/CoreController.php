...
// Le mot clé abstract devant la classe permet de la rendre abstraite. C'est à dire qu'on ne pourra pas l'instancier. Cette opération a pour but de rendre notre code mieux structure, les futures personnes qui travailleront dessus auront une erreur si elles essaient d'instancier cette classe.
abstract class CoreController
{
    ...
    */
    public function __construct($routeName, $router)
    {
        ...
        // dump($routeName);
        // On se crée un tableau d'ACL (access control list) qui centralise la liste des routes qui doivent etre protégées et les roles autorisés sur ces routes
        $acl = [
            'user-list' => ['admin'],
            'user-add' => ['admin'],
            'user-create' => ['admin'],
            ...
            'category-managehome' => ['admin', 'catalog-manager'],
        ];
        ...
        // On va pouvoir ensuite vérifier si la page courante est protégée contre les attaques CSRF.
        ...
        // On vérifie que la route sur laquelle on se trouve est protégée contre les attaques CSRF
        if(isset($csrfRoutes[$routeName])) {

            // On récupère le token de session de l'utilisateur
            // La notation "??" permet de vérifier si l'entrée token existe dans le tableau $_SESSION et de récupérer son contenu. Si ce n'est pas le cas, alors $sessionToken contient une chaine vide.
            $sessionToken = $_SESSION['token'] ?? '';

            // On doit récupérer le token qui provient du formulaire. Mais on ne sait pas encore si on le recupère depuis $_GET ou $_POST. Donc on doit vérifier sur quel type de route on est.
            $httpMethod = $csrfRoutes[$routeName];
            ...
            // Une fois les deux tokens retrouvés, on vérifie qu'ils sont bien présents et qu'ils sont identiques
            if(empty($sessionToken) || empty($formToken) || $formToken != $sessionToken) {
    ...
    /**
     * Génère une clé aléatoire, la stocke dans la session et la retourne
     *
     * @return string
     */
    public function generateCSRFToken()
    {
        ...
        // On stocke une copie de ce token dans la session
